...
| `CapacidadeTotal` | `INT NOT NULL` | Número máximo de ingressos |
| `DataEvento` | `DATETIME NOT NULL` | Data e hora de realização |
| `PrecoPadrao` | `DECIMAL(10,2) NOT NULL` | Valor unitário do ingresso |
...
| Campo | Tipo | Descrição |
|-------|------|-----------|
| `Codigo` | `VARCHAR(50) NOT NULL` | Código alfanumérico do cupom (PK) |
| `PorcentagemDesconto` | `DECIMAL(5,2) NOT NULL` | Percentual de desconto (ex.: 10.00 = 10%) |
| `ValorMinimoRegra` | `DECIMAL(10,2) NOT NULL` | Valor mínimo do ingresso para aplicar o desconto |
...
| `CupomUtilizado` | `VARCHAR(50) NULL` | FK para Cupons(Codigo) — opcional |
...
1. **Limite por CPF:** cada CPF pode reservar no máximo **2 ingressos por evento**.
2. **Controle de capacidade:** uma reserva só pode ser confirmada se ainda houver ingressos disponíveis para o evento.
3. **Cupons:** um cupom pode ser aplicado opcionalmente a uma reserva. O desconto só é aplicado quando o `PrecoPadrao` do evento for maior ou igual ao `ValorMinimoRegra` do cupom.
4. **Cálculo do valor final:** `ValorFinalPago = PrecoPadrao × (1 - PorcentagemDesconto / 100)`. Se nenhum cupom for utilizado, `ValorFinalPago = PrecoPadrao`.
5. **Integridade referencial:** toda reserva deve estar associada a um usuário e a um evento existentes. Se um cupom for informado, ele também deve existir na base de cupons; caso contrário, a reserva deve ser rejeitada.
...
**Quero** cadastrar um novo evento informando nome, data do evento, capacidade total e preço padrão,
...
Dado que o organizador informou nome "Show da Banda X", data do evento "2026-12-15 20:00", capacidade total 500 e preço padrão R$ 150,00
...
E que o usuário com CPF "12345678901" está cadastrado
E que existe um cupom "DESCONTO10" com 10% de desconto e ValorMinimoRegra de R$ 0,00
...
**Cenário 3: Reserva rejeitada por CPF com limite excedido no mesmo evento**

Dado que o usuário com CPF "12345678901" já possui 2 reservas para o evento "Show da Banda X"
Quando o mesmo usuário tentar reservar um terceiro ingresso para o mesmo evento
Então o sistema deve rejeitar a reserva com mensagem de erro informando que o CPF atingiu o limite de reservas para este evento

**Cenário 4: Reserva rejeitada por capacidade esgotada**

Dado que o evento "Show da Banda X" possui capacidade para 1 pessoa
E que outro usuário já possui uma reserva confirmada para este evento
...
**Cenário 5: Reserva com cupom sem desconto por valor mínimo não atingido**

Dado que existe um evento com preço padrão de R$ 80,00
E que existe um cupom "DESC10" com 10% de desconto e ValorMinimoRegra de R$ 100,00
Quando o usuário solicitar a reserva utilizando o cupom "DESC10"
Então o sistema deve criar a reserva com valor final de R$ 80,00
E o sistema não deve aplicar o desconto pois o preço do ingresso é inferior ao valor mínimo do cupom
...
**Cenário 7: Reserva rejeitada por cupom inexistente**

Dado que existe um evento "Show da Banda X" com preço de R$ 150,00
E que o usuário com CPF "12345678901" está cadastrado
E que o cupom "CUPOMINEXISTENTE" não existe no sistema
Quando o usuário solicitar a reserva de 1 ingresso utilizando o cupom "CUPOMINEXISTENTE"
Então o sistema deve rejeitar a reserva com mensagem de erro informando que o cupom não foi encontrado
...
Dado que o usuário com CPF "12345678901" possui 3 reservas, cada uma em um evento diferente
...
**Como** organizador de eventos,
**Quero** cadastrar cupons de desconto com código, percentual e valor mínimo do ingresso,
...
Dado que o organizador informou código "PROMO20", desconto de 20% e ValorMinimoRegra do ingresso de R$ 50,00
Quando a requisição de cadastro for enviada  
...
| **Limite por CPF** | Restrição que permite no máximo 2 reservas por CPF por evento |
...
| 1.1.0 | 2026-05-14 | Alinhamento ao enunciado oficial da AV2: limite por CPF alterado de 1 para 2 reservas por evento |
